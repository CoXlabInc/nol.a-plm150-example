...
static void eventSerialReceived(SerialPort &) {
  ...
  if (strcmp(keyBuf, "length") == 0) {
    ...
    if (len > 0) {
      ...
      if (dump) {
        ConfigMemory.read(dump, addr, len);
    ...
    ConfigMemory.write(dataBlock, addr, 1024);
    ...
    ConfigMemory.write((const uint8_t *) data, addr, strlen(data));
  } else {
